...
import { COUNTRY_MAP } from "@/data/ktrend/meta";
import { CONTENT, fmtCompact, sortContent } from "@/data/ktrend/content";

export default function ViralPage() {
  const topViral = sortContent(CONTENT, "viral").slice(0, 12);
  const signals = topViral.slice(0, 5).map((c, i) => {
    const brand = BRAND_MAP[c.brandId];
    const country = COUNTRY_MAP[c.country];
    const surge = 120 + ((c.viralScore * 3) % 280);
    return {
      id: c.id,
      level: c.viralScore >= 85 ? "high" : i % 2 === 0 ? "mid" : "low",
      text: `${brand?.nameKo} ${c.subCategory} ${country.nameKo} 샵 조회수 ${surge}% 급증!`,
      sub: `${c.style.toUpperCase()} · 누적 ${fmtCompact(c.views)} 조회 · 추정 ROAS ${c.estRoasX}x`,
    };
  });
      ...
      <div className="grid gap-4 lg:grid-cols-3">
          ...
          <div className="space-y-2">
            {signals.map((s) => (
              <div key={s.id} className={`flex items-start gap-3 rounded-lg border px-4 py-3 ${levelStyle[s.level]}`}>
                ...
                <button className="kt-btn ml-auto bg-white/70 px-2.5 py-1 text-[10px] font-bold">
                  대응 크리에이터 매칭
                </button>
              </div>
            ))}
        ...
        <div className="kt-card p-4">
          ...
          <div className="space-y-3 text-[11px]">
            ...
            <Condition label="바이럴 점수 기준" value="75 이상" />
