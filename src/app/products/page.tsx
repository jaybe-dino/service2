...
export default function ProductsPage() {
  ...
  useEffect(() => {
    let alive = true;
    setLoading(true);
    fetch(`/api/products?sort=${sort}&limit=300`)
      .then((r) => r.json())
      .then((d) => { if (!alive) return; setProducts(Array.isArray(d.products) ? d.products : []); setConfigured(d.configured !== false); })
              ...
              <tbody>
                {rows.map((p, i) => (
                  <tr key={p.id} className="border-t border-slate-100 hover:bg-slate-50/60">
                    ...
                    <td className="max-w-[280px] p-2.5 font-semibold">{p.title || <span className="text-slate-300">(제목 없음)</span>}</td>
