...
const SAMPLE_BRANDS = ["샘플브랜드 A", "샘플브랜드 B", "샘플브랜드 C"];
...
export async function POST(req: Request) {
  ...
  if (body.clear) {
    ...
    await sql`DELETE FROM brand_shop_stats WHERE brand_name = ANY(${SAMPLE_BRANDS as unknown as string})`;
  ...
  for (const brand of SAMPLE_BRANDS) {
    ...
    total += await ingestProducts(brand, products);
  }
  return NextResponse.json({ ok: true, seeded: total, brands: SAMPLE_BRANDS, note: "샘플 데이터입니다. {clear:true}로 제거하세요." });
}
